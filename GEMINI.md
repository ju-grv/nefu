...
- **CSS**:
    ...
    - **CSS編集の原則**:
        - **既存スタイルの上書きは避ける:** 既存のCSSルールを広範囲に置き換えるのではなく、より詳細度の高いセレクタ（例: `.new-section .some-class`）を持つ新しいルールを追加して、特定の変更を行うこと。これにより、意図しない他の要素への影響を防ぎます。
        - **変更はスコープを限定する:** 新しいレイアウトやコンポーネントを追加する際は、親要素に固有のクラス（例: `.features-section`）を付け、すべての関連CSSセレクタをそのクラスで始めることで、スタイルの影響範囲を限定すること。
- **JavaScript**:
    - 変数命名: camelCase
    - コメント: JSDocスタイル
    - ESLint ルール
